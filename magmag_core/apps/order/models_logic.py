...
class OrderCheckout(object):
    ...
    def process_form(view, form):
        lst = deserialize_list(unquote(form.cleaned_data['json_purchase_pi'])) if 'json_purchase_pi' in form.cleaned_data \
            else list()
        pis = dict([(x['id'], x['count']) for x in lst])
        products = list(ProductItem.objects.select_related('product').filter(id__in=pis.keys()))
        total_sum = sum(el.product.price*pis[el.id] for el in products)

        customer = Profile.objects.filter(email=form.cleaned_data['email']).first()
        if customer is None:
            customer = Profile()
        ...
        for prod in products:
            pi = PurchaseItem().initialize(pis[prod.id], prod)
            order.items.add(pi)
            pi_list.append(pi)
